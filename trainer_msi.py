...
class Trainer():
    ...
    def train(self, current_epoch=0):
        ...
        for i_batch, sample_batched in enumerate(tqdm(self.dataloader['train'])):
            ...
            outputs = self.model(hrmsi)
            loss = self.criterion(outputs, hrmsi)
        ...
        self.viz.line(X=np.array([current_epoch]), Y=np.array([np.mean(train_loss)]), win='msi_tarin_loss_win',
                      opts={'title': 'msi_train_loss', 'legend': ['msi_train']},
    ...
    def evaluate(self, current_epoch=0):
        ...
        with torch.no_grad():
            ...
            self.viz.line(X=np.array([current_epoch]), Y=np.array([np.mean(eval_loss)]), win='msi_eval_loss_win',
                          opts={'title': 'msi_eval_loss', 'legend': ['msi_eval']},
